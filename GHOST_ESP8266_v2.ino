...
void fetchParametersFromServer() {
    ...
    if (httpCode == HTTP_CODE_OK) {
        ...
        if (!err) {
            ...
            if (doc.containsKey("target_humidity")) savedTargetHum = doc["target_humidity"];
            else if (doc.containsKey("target_hum")) savedTargetHum = doc["target_hum"];
            if (doc.containsKey("min_humidity")) savedMinHum = doc["min_humidity"];
            else if (doc.containsKey("min_hum")) savedMinHum = doc["min_hum"];
            if (doc.containsKey("max_humidity")) savedMaxHum = doc["max_humidity"];
            else if (doc.containsKey("max_hum")) savedMaxHum = doc["max_hum"];
            if (doc.containsKey("turning_interval")) savedTurningInt = doc["turning_interval"];
            if (doc.containsKey("turning_lockdown_active")) turningLockdownActive = doc["turning_lockdown_active"];
            if (doc.containsKey("turning_lockdown_days_remaining")) turningLockdownDaysRemaining = doc["turning_lockdown_days_remaining"];
            if (doc.containsKey("swing_duration_sec")) {
                int duration = doc["swing_duration_sec"];
                if (duration >= 5 && duration <= 300) {
                    savedSwingDurationSec = duration;
                }
            }
            if (doc.containsKey("active_session_name")) strncpy(sessionName, doc["active_session_name"], 49);
            else if (doc.containsKey("session_name")) strncpy(sessionName, doc["session_name"], 49);
            ...
            Serial.printf("[Server] session_status=%s session_name=%s session_id=%d\n",
                ...
                doc["active_session_name"] | (doc["session_name"] | ""),
                doc["session_id"] | 0);
