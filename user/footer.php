...
<div id="ajaxLoader" style="display:none;position:fixed;inset:0;z-index:88888;background:rgba(10,12,16,0.75);backdrop-filter:blur(4px);-webkit-backdrop-filter:blur(4px);align-items:center;justify-content:center;flex-direction:column;gap:16px;">
  <div style="position:relative;width:60px;height:60px;display:flex;align-items:center;justify-content:center;">
    <div style="position:absolute;inset:0;border-radius:50%;border:3px solid rgba(255,255,255,0.06);border-top-color:#3b82f6;animation:spinRing 0.8s linear infinite;"></div>
    <div style="position:absolute;inset:10px;border-radius:50%;border:3px solid rgba(255,255,255,0.04);border-bottom-color:#6366f1;animation:spinRing 1.1s linear infinite reverse;"></div>
    <div style="width:8px;height:8px;border-radius:50%;background:#3b82f6;animation:loaderPulse2 1s ease infinite;"></div>
  </div>
  <div id="ajaxLoaderText" style="font-family:'Space Grotesk',sans-serif;font-size:0.8rem;font-weight:600;color:#64748b;letter-spacing:0.1em;text-transform:uppercase;animation:textPulse2 1.4s ease infinite;">Saving…</div>
</div>
<style>
...
function toggleSidebarCollapse(){
  ...
  try{ localStorage.setItem('ghost_user_sidebar_collapsed', collapsed ? '1' : '0'); }catch(e){}
}
// Restore sidebar state on load
try{
  if(localStorage.getItem('ghost_user_sidebar_collapsed') === '1'){
...
// ── PAGE TRANSITION (intercept all nav link clicks — skip logout modal, modals, hash) ──
...
(function(){
  ...
  async function runTick() {
    ...
    console.info('[GHOST scheduler] check running', { source: 'user-page-poll' });
    try {
