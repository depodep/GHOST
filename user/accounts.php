    ...
    <div class="ghost-panel h-100">
      ...
      <div class="ghost-panel-body">
        <div style="display:flex;align-items:center;gap:14px;margin-bottom:16px;">
          <div style="width:54px;height:54px;border-radius:14px;background:linear-gradient(135deg,#3b82f6,#6366f1);display:flex;align-items:center;justify-content:center;font-weight:700;font-size:1.2rem;color:white;"> <?= strtoupper(substr($user['full_name'],0,1)) ?> </div>
          <div>
            <div style="font-size:1.1rem;font-weight:700;color:white;"><?= htmlspecialchars($user['full_name']) ?></div>
            <div style="font-size:.8rem;color:var(--ghost-muted);">Farm User</div>
          </div>
        </div>
        <div class="mb-2"><span style="color:var(--ghost-muted);font-size:.75rem;text-transform:uppercase;letter-spacing:.1em;">Email</span><div style="font-weight:600;color:white;"><?= htmlspecialchars($user['email']) ?></div></div>
        <div class="mb-2"><span style="color:var(--ghost-muted);font-size:.75rem;text-transform:uppercase;letter-spacing:.1em;">Phone</span><div style="font-weight:600;color:white;"><?= htmlspecialchars($user['phone'] ?? '—') ?></div></div>
        <?php $accountStatus = strtolower((string)($user['status'] ?? 'active')); ?>
        <div class="mb-2"><span style="color:var(--ghost-muted);font-size:.75rem;text-transform:uppercase;letter-spacing:.1em;">Account Status</span><div id="accountStatusText" style="font-weight:600;color:<?= $accountStatus === 'active' ? '#22c55e' : ($accountStatus === 'suspended' ? '#ef4444' : '#94a3b8') ?>;"><?= htmlspecialchars(ucfirst($accountStatus)) ?></div></div>
        <div><span style="color:var(--ghost-muted);font-size:.75rem;text-transform:uppercase;letter-spacing:.1em;">Joined</span><div style="font-weight:600;color:white;"><?= date('M j, Y', strtotime($user['created_at'])) ?></div></div>
      </div>
    </div>
  </div>
</div>
...
function applyRelayCommandState(incubatorId, relay, state) {
  const liveState = relayLiveState[incubatorId] || { heater: false, fan: false, swing: false, exhaust: false };
  if (relay === 'heater') {
